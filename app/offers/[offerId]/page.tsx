...
import { getTeswaConditionLabel } from "@/lib/teswa-product-language";
...
export default async function OfferDetail({ params, searchParams }: { params: Promise<{ offerId: string }>; searchParams?: Promise<{ error?: string; response?: string; reported?: string }> }) {
  ...
  const reqOwner = firstOrNull(requested.profiles)?.display_name ?? "صاحب الحاجة";
  ...
  return <PageShell className="mx-auto max-w-5xl px-4 py-10"><PageSection className="space-y-6">
    <HeroPanel className="space-y-3"><h1 className="text-3xl font-bold">اقتراح قيمة</h1><OfferStatusBadge status={offer.status} /><p className="text-sm text-app-text-muted">{isReceiver ? "راجع الاقتراح وشوف هل هو يفتح معنى مناسب للحاجة." : "تابع اقتراحك واعرف رد صاحب الحاجة والخطوة الجاية."}</p></HeroPanel>
    ...
    <SurfaceCard className="grid gap-4 p-4 md:grid-cols-[1fr_auto_1fr]"><OfferItemCard itemId={offered.id} title={offered.title} imageUrl={offImg} category={firstOrNull(offered.categories)?.name_ar ?? null} conditionLabel={getTeswaConditionLabel(offered.condition)} ownerName={offOwner} /><div className="self-center text-center text-3xl">↔</div><OfferItemCard itemId={requested.id} title={requested.title} imageUrl={reqImg} category={firstOrNull(requested.categories)?.name_ar ?? null} conditionLabel={getTeswaConditionLabel(requested.condition)} ownerName={reqOwner} /></SurfaceCard>
    <SoftPanel><p className="text-stone-700">{offProfile?.username ? <Link href={`/users/${offProfile.username}`} className="hover:underline">{offOwner}</Link> : offOwner} شايف إن {offered.title} ممكن تبقى إجابة مناسبة على {requested.title} عند {reqProfile?.username ? <Link href={`/users/${reqProfile.username}`} className="hover:underline">{reqOwner}</Link> : reqOwner}.</p></SoftPanel>
    {offer.message ? <div className="rounded-xl border p-3"><p className="font-semibold">الرسالة اللي وصل بها الاقتراح:</p><p>«{offer.message}»</p></div> : null}
    ...
    {offer.parent_offer_id ? <div className="rounded-xl border border-sky-200 bg-sky-50 p-3"><p className="font-semibold">اقتراح جديد بعد فتح باب تاني</p><Link className="text-sm underline" href={`/offers/${offer.parent_offer_id}`}>افتح العرض الأصلي</Link></div> : null}
    ...
    {isSender && offer.status === "pending" ? <p className="rounded-xl bg-blue-50 p-3 text-blue-900">اقتراحك وصل. مستني رد صاحب الحاجة.</p> : null}
    {isSender && offer.status === "thinking" ? <p className="rounded-xl bg-yellow-50 p-3 text-yellow-900">صاحب الحاجة شاف الاقتراح ومحتاج يفكر.</p> : null}
    {isSender && offer.status === "accepted" ? <p className="rounded-xl bg-emerald-50 p-3 text-emerald-900">اقتراحك اتقبل. افتح صفحة التنسيق واتفقوا بهدوء.</p> : null}
    {isSender && offer.status === "soft_rejected" ? <p className="rounded-xl bg-stone-100 p-3 text-stone-700">الاقتراح ما كانش مناسب المرة دي. ممكن تجرّب حاجة تانية.</p> : null}
    {isSender && offer.status === "redirected" ? <div className="space-y-2 rounded-xl bg-sky-50 p-3 text-sky-900"><p className="font-semibold">صاحب الحاجة فتح بابًا مختلفًا. جهّز اقتراحًا جديدًا بحاجة مختلفة.</p><p>العرض الجديد لازم يكون بحاجة مختلفة عن العرض الأول.</p><Link href={`/offers/new?fromOffer=${offer.id}`} className="inline-flex rounded-lg bg-sky-700 px-3 py-2 text-sm text-white">ابعت اقتراح تاني</Link></div> : null}

    {isReceiver && offer.status === "accepted" ? <p className="rounded-xl bg-emerald-50 p-3 text-emerald-900">أنت وافقت على الاقتراح. تقدروا تبدأوا التنسيق من الصفحة الخاصة.</p> : null}
    {isReceiver && offer.status === "soft_rejected" ? <p className="rounded-xl bg-stone-100 p-3 text-stone-700">أنت قفلت الاقتراح بلطف.</p> : null}
    {isReceiver && offer.status === "redirected" ? <p className="rounded-xl bg-sky-50 p-3 text-sky-900">أنت فتحت بابًا تانيًا لصاحب الاقتراح. لو بعت شيئًا جديدًا هيظهر هنا.</p> : null}
