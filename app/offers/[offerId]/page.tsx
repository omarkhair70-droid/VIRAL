...
export default async function OfferDetail({ params }: { params: Promise<{ offerId: string }> }) {
  ...
  return <section className="mx-auto max-w-5xl space-y-6 px-4 py-10">
    ...
    <div className="rounded-xl bg-stone-50 p-4"><p className="font-semibold">الخط الزمني</p><ul className="mt-2 list-disc space-y-1 pr-4"><li>العرض اتبعت - {new Date(offer.created_at).toLocaleDateString("ar-EG")}</li><li>لسه مستني رد صاحب الحاجة</li></ul></div>
    {user?.id === offer.receiver_id ? <div className="rounded-xl border border-amber-200 bg-amber-50 p-3"><p className="mb-2">الرد على العرض جاي في المرحلة الجاية:</p><div className="flex flex-wrap gap-2">{["قبول", "محتاج أفكر", "العرض ما ظبطش"].map((label) => <button disabled key={label} className="rounded-lg border px-3 py-2 text-sm opacity-70">{label} - قريبًا</button>)}</div></div> : null}
