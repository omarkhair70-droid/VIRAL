  ...
  return <div className="space-y-4">
    ...
    <SurfaceCard className={`space-y-4 ${mode === "new_item" ? "" : "hidden"}`}><h3 className="text-lg font-semibold">جهّز الحاجة الجديدة كعرض</h3>
      <FormSection>
        ...
        <Field><Label htmlFor="condition">حالة الحاجة</Label><Select id="condition" name="condition" defaultValue="good_used"><option value="almost_new">{TESWA_CONDITION_LANGUAGE.almost_new.label}</option><option value="good_used">{TESWA_CONDITION_LANGUAGE.good_used.label}</option><option value="minor_issues">{TESWA_CONDITION_LANGUAGE.minor_issues.label}</option><option value="needs_repair">{TESWA_CONDITION_LANGUAGE.needs_repair.label}</option></Select></Field>
        ...
        <Field><Label htmlFor="desire_mode">فاتح الباب لإيه؟</Label><Select id="desire_mode" name="desire_mode" defaultValue="flexible"><option value="specific">{TESWA_DESIRE_MODE_LANGUAGE.specific.label}</option><option value="flexible">{TESWA_DESIRE_MODE_LANGUAGE.flexible.label}</option><option value="surprise">{TESWA_DESIRE_MODE_LANGUAGE.surprise.label}</option></Select></Field>
