...
export default async function NewOfferPage({ searchParams }: { searchParams: Promise<{ requestedItemId?: string; fromOffer?: string; error?: string }> }) {
  ...
  if (!req || req.status !== "active") return <PageShell><StateBlock title="الحاجة دي مش متاحة للعروض دلوقتي." tone="danger" primaryAction={<ButtonLink href="/items" variant="outline" size="sm">ارجع للاستكشاف</ButtonLink>} /></PageShell>;
  if (req.owner_id === user.id || params.error === "own_item") return <PageShell><StateBlock title="دي حاجتك أنت. مينفعش تعرض على حاجة بتاعتك." tone="info" primaryAction={<ButtonLink href="/items" variant="outline" size="sm">ارجع للاستكشاف</ButtonLink>} /></PageShell>;
  ...
  return <PageShell className="max-w-6xl">
    <PageSection>
      <HeroPanel>
        <p className="text-sm font-semibold text-clay">ردّ على سؤال القيمة</p>
        <h1 className="text-2xl font-bold">قول دي تِسوى إيه عندك</h1>
        <p className="mt-2 text-app-text-secondary">اختار حاجة من عالمك، أو جهّز واحدة جديدة، وخلّي اقتراحك يشرح ليه الباب ده مناسب.</p>
      </HeroPanel>
      {params.error && errorMap[params.error] ? <InlineNotice tone="danger">{errorMap[params.error]}</InlineNotice> : null}
      {sourceOffer ? <HighlightPanel className="space-y-2"><p className="font-semibold">دي محاولة جديدة بعد ما صاحب الحاجة فتح بابًا مختلفًا.</p><p className="text-sm text-app-text-secondary">لازم تعرض حاجة مختلفة عن عرضك الأول.</p>{sourceOffer.public_note ? <SoftPanel><p className="text-sm">ملاحظة صاحب الحاجة: {sourceOffer.public_note}</p></SoftPanel> : null}{sourceOffer.redirect_type ? <p className="text-xs text-app-text-muted">نوع الباب التاني: {redirectMap[sourceOffer.redirect_type] ?? sourceOffer.redirect_type}</p> : null}</HighlightPanel> : null}

      <div className="grid gap-5 lg:grid-cols-[1.1fr_1.9fr]">
        <SurfaceCard className="h-fit space-y-3">
          <p className="text-sm font-semibold text-clay">الحاجة اللي شدت عينك</p>
          <MediaFrame src={reqImg} alt={req.title} ratio="wide" />
          <h2 className="text-xl font-bold">{req.title}</h2>
          <p className="text-sm text-app-text-muted">{reqCat?.name_ar ?? "بدون تصنيف"} · {getTeswaConditionLabel(req.condition)}</p>
          <p className="text-sm">صاحبها: {reqOwner?.display_name ?? "مستخدم"}</p>
          <SoftPanel><p className="text-sm">الباب اللي صاحبها فتحه: {req.desire_text ? `صاحب الحاجة سايب لك المساحة دي: ${req.desire_text}` : "مافيش توضيح مكتوب. حاول تشرح في رسالتك ليه اقتراحك مناسب."}</p></SoftPanel>
        </SurfaceCard>
        ...
        <form action={createOffer} encType="multipart/form-data">
          ...
          <OfferComposerClient requestedItem={{ id: req.id, title: req.title, conditionLabel: getTeswaConditionLabel(req.condition), category: reqCat?.name_ar ?? null, ownerName: reqOwner?.display_name ?? "مستخدم", imageUrl: reqImg, desireText: req.desire_text }} ownItems={filteredOwnItems.map((it) => ({ id: it.id, title: it.title, conditionLabel: getTeswaConditionLabel(it.condition as "almost_new"|"good_used"|"minor_issues"|"needs_repair"), category: firstOrNull(it.categories)?.name_ar ?? null, imageUrl: it.item_images?.find((img) => img.is_primary)?.image_url ?? it.item_images?.[0]?.image_url ?? null }))} categories={(categories ?? []).map((c) => ({ id: c.id, name_ar: c.name_ar }))} hasSourceOffer={Boolean(sourceOffer)} />
        </form>
      </div>
    </PageSection>
  </PageShell>;
