    ...
    <form action={updateProfile} className="space-y-5 rounded-2xl border bg-white p-5" encType="multipart/form-data">
      <div className="space-y-3">
        ...
        <div className="grid gap-4 sm:grid-cols-2">
          <div><p className="mb-2 text-sm font-medium">الأفاتار</p>{profile?.avatar_url ? <Image src={profile.avatar_url} alt="avatar" width={88} height={88} className="mb-2 h-[88px] w-[88px] rounded-full object-cover" /> : <div className="mb-2 flex h-[88px] w-[88px] items-center justify-center rounded-full border bg-stone-100 text-sm">بدون صورة</div>}<input type="file" name="avatar_file" accept="image/jpeg,image/png,image/webp" className="w-full text-sm" /></div>
          <div><p className="mb-2 text-sm font-medium">صورة الغلاف</p>{profile?.cover_url ? <Image src={profile.cover_url} alt="cover" width={320} height={88} className="mb-2 h-[88px] w-full rounded-xl object-cover" /> : <div className="mb-2 flex h-[88px] w-full items-center justify-center rounded-xl border bg-stone-100 text-sm">بدون غلاف</div>}<input type="file" name="cover_file" accept="image/jpeg,image/png,image/webp" className="w-full text-sm" /></div>
        </div>
      </div>
