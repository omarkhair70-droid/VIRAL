...
type FeaturedRow = { items: { id:string; title:string; condition:"almost_new"|"good_used"|"minor_issues"|"needs_repair"; city:string|null; area:string|null; desire_mode:"specific"|"flexible"|"surprise"; desire_text:string|null; item_story:string|null; swap_reason:string|null; good_for:string|null; categories:{name_ar:string|null}[]|null; item_images:{image_url:string|null; is_primary:boolean|null}[]|null }[] };
type DropRow = { id:string; title:string; drop_type:string; creator_name:string|null; intro_copy:string; creator_drop_items:{ sort_order:number; items:{id:string; title:string}[] }[]|null };
...
export default async function DropsPage() {
  ...
  const featuredItems = featuredRows.flatMap((row) => row.items).map((item) => ({ ...item, categoryName: item.categories?.[0]?.name_ar ?? null, imageUrl: item.item_images?.find((img) => img.is_primary)?.image_url ?? item.item_images?.[0]?.image_url ?? null, hasStory: true }));
