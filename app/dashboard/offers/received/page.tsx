...
export default async function ReceivedOffersPage({ searchParams }: { searchParams: Promise<{ tab?: string }> }) {
  ...
  const { data } = await supabase
    .from("offers")
    .select("id,status,created_at,requested_item:items!offers_requested_item_id_fkey(title),offered_item:items!offers_offered_item_id_fkey(title),sender:profiles!offers_sender_id_fkey(display_name)")
  ...
  return <PageShell title="عروض وصلتني"><div className="mb-4 flex flex-wrap gap-2">{[["all","الكل"],["pending","مستنية رد"],["thinking","محتاج تفكير"],["accepted","اتقبلت"],["soft_rejected","ما ظبطتش"],["redirected","اتفتح باب تاني"]].map(([key,label])=><Link key={key} href={key==="all"?"/dashboard/offers/received":`/dashboard/offers/received?tab=${key}`} className="rounded-lg border px-3 py-1.5 text-sm">{label}</Link>)}</div>{offers.length===0?<p className="rounded-xl border bg-white p-4">لسه ما وصلكش عروض.</p>:<div className="grid gap-3">{offers.map((offer)=><AccountOfferCard key={offer.id} offer={offer} sideLabel="من" ctaLabel={offer.status === "pending" || offer.status === "thinking" ? "افتح ورد" : "افتح العرض"} />)}</div>}</PageShell>;
}
