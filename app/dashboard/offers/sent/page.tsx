...
return <PageShell title="اقتراحات بعتها"><PageSection><HeroPanel><h1 className="text-2xl font-semibold">اقتراحات بعتها</h1><p className="mt-1 text-sm text-app-text-secondary">تابع حالة اقتراحاتك واعرف رد الطرف التاني.</p></HeroPanel><SoftPanel className="flex flex-wrap gap-2">{[["all","الكل"],["pending","مستنية رد"],["thinking","محتاج تفكير"],["accepted","اتقبلت"],["soft_rejected","ما ظبطتش"],["redirected","اتفتح باب تاني"]].map(([k,l])=><Link key={k} href={k==="all"?"/dashboard/offers/sent":`/dashboard/offers/sent?tab=${k}`} className={chipClass(tab === k)}>{l}</Link>)}</SoftPanel>{offers.length===0?<SoftPanel><p className="font-semibold">لسه ما بعتش اقتراحات.</p><p className="mt-1 text-sm text-app-text-muted">افتح احتمالًا مناسبًا من الاستكشاف وابدأ اقتراحًا جديدًا.</p><ButtonLink href="/items" variant="secondary" size="sm" className="mt-3">استكشف الاحتمالات</ButtonLink></SoftPanel>:<div className="grid gap-3">{offers.map((offer)=><AccountOfferCard key={offer.id} offer={offer} sideLabel="إلى" ctaLabel="افتح الاقتراح" />)}</div>}</PageSection></PageShell>;
}
