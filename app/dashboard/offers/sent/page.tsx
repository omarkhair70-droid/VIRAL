...
export default async function SentOffersPage({ searchParams }: { searchParams: Promise<{ tab?: string }> }) {
  ...
  return <PageShell title="عروض بعتها"><div className="mb-4 flex flex-wrap gap-2">{[["all","الكل"],["pending","مستنية رد"],["thinking","محتاج تفكير"],["accepted","اتقبلت"],["soft_rejected","ما ظبطتش"],["redirected","اتفتح باب تاني"]].map(([key,label])=><Link key={key} href={key==="all"?"/dashboard/offers/sent":`/dashboard/offers/sent?tab=${key}`} className="rounded-lg border px-3 py-1.5 text-sm">{label}</Link>)}</div>{offers.length===0?<div className="rounded-xl border bg-white p-5"><p className="font-semibold">لسه ما بعتش عروض.</p><p className="mt-1 text-sm text-stone-600">ابدأ من السوق واختار حاجة مناسبة وابعت عرض محترم وواضح.</p><Link href="/items" className="mt-3 inline-flex rounded-lg border px-3 py-2 text-sm">شوف السوق</Link></div>:<div className="grid gap-3">{offers.map((offer)=><AccountOfferCard key={offer.id} offer={offer} sideLabel="إلى" ctaLabel="افتح العرض" />)}</div>}</PageShell>;
}
