...
export default async function DashboardItemsPage({ searchParams }: { searchParams: Promise<{ tab?: string }> }) {
  ...
  return <PageShell title="حاجاتي"><PageSection>
    ...
    <SoftPanel className="flex flex-wrap items-center gap-2"><Link href="/dashboard/items" className="rounded-full border px-3 py-1.5 text-sm">الكل</Link><Link href="/dashboard/items?tab=active" className="rounded-full border px-3 py-1.5 text-sm">نشطة</Link><Link href="/dashboard/items?tab=archived" className="rounded-full border px-3 py-1.5 text-sm">مؤرشفة</Link><span className="ms-auto flex items-center gap-2 text-sm text-app-text-muted">النتايج <CountBadge count={items.length} /></span></SoftPanel>
